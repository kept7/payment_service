...
email_str_unique = Annotated[
    str, mapped_column(String(255), unique=True, nullable=False, index=True)
]

email_str = Annotated[str, mapped_column(String(255), nullable=False)]
...
PaymentStatus = Literal["Создан", "Оплачен", "Отменен"]
payment_status = Annotated[
    PaymentStatus,
    mapped_column(
        Enum("Создан", "Оплачен", "Отменен", name="payment_status_enum"),
        nullable=False,
    ),
]
